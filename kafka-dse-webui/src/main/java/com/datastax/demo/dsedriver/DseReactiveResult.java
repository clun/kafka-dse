...
public class DseReactiveResult < M > {
    ...
    private Flux<M> getRecords(Mono<Result<M>> nextResults) {
        return nextResults.flatMapMany(it -> {
            ...
            return rows.doOnComplete(() -> DseReactiveResult.fetchMoreRecords(it.fetchMoreResults(), processor))
    ...
    static public <T> void fetchMoreRecords(ListenableFuture<Result<T>> future, MonoProcessor<Result<T>> sink) {
        try {
            future.addListener(() -> {
                try {
