...
/**
 * Provides a way to convert objects returns by Java Driver to Mono<T> Flux<T>
 * expected by Reactor and Spring WebFlux.
 * 
...
public abstract class DseReactiveDaoSupport implements DseConstants {
    ...
    protected Mono<DseReactiveResultSet> executeASync(ListenableFuture<ResultSet> future) {
        return Mono.<DseReactiveResultSet>create(sink -> {
            try {
                ListenableFuture<DseReactiveResultSet> resultSetFuture = Futures.transform(future, DseReactiveResultSet::new);
    ...
    protected <T> Mono<DseReactiveResult<T>> executeAndMapASync(ListenableFuture<Result<T>> resultAsyncMapper) {
        return Mono.<DseReactiveResult<T>>create(sink -> {
            try {
                
                ListenableFuture<DseReactiveResult<T>> resultSetFuture = Futures.transform(resultAsyncMapper, 
                        DseReactiveResult<T>::new);
    ...
    protected <T> Flux<T> asFlux(ListenableFuture<Result<T>> futureDriver) {
        return executeAndMapASync(futureDriver)
                ...
                .onErrorMap(throwable -> new IllegalStateException("Error during Select ticker info", throwable));
    }
