...
public class DseReactiveResultSet {
     ...
     *
     * @return
     *      return flux of Rows with no mapping
     */
    ...
    private Flux<Row> getRows(Mono<ResultSet> nextResults) {
        return nextResults.flatMapMany(it -> {
            ...
            return rows.doOnComplete(() -> DseReactiveResultSet.fetchMoreRows(it.fetchMoreResults(), processor))
    ...
    static public void fetchMoreRows(ListenableFuture<ResultSet> future, MonoProcessor<ResultSet> sink) {
        try {
            future.addListener(() -> {
                try {
