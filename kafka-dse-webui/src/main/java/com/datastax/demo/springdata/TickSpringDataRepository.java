 ...
 */
public interface TickSpringDataRepository extends ReactiveCrudRepository<TickData, TickDataPrimaryKey> {
    
    @Query("SELECT * FROM " + DseConstants.STOCKS_TICKS + " WHERE symbol = ?0 LIMIT 100")
    Flux< TickData > findBySymbol(String symbol);
    
    @Query("SELECT * FROM " + DseConstants.STOCKS_TICKS + " WHERE symbol in ('BAC', 'DVMT','DIS','IBM','WMT') LIMIT 500")
